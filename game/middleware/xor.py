    ...
    def before(cls, session, request):
        if session.encryption_enabled:
            ...
            for i in range(0, len(request.data)):
                temp2 = Int32(request.data[i]) & 255
                request.data[i] = Int8(
        ...
        if session.encryption_enabled:
            ...
            for i in range(0, len(response.data)):
                ...
                response.data[i] = Int8(
                    temp2 ^ session.xor_key.outgoing_key[i & 15] ^ temp
                )
                temp = response.data[i]

            old = Int32(session.xor_key.outgoing_key[8]) & 0xFF
            old |= Int32(session.xor_key.outgoing_key[9] << 8) & 0xFF00
            old |= Int32(session.xor_key.outgoing_key[10] << 16) & 0xFF0000
            old |= Int32(session.xor_key.outgoing_key[11] << 24) & 0xFF000000
